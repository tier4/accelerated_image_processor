...
cudaError_t cudaBGRToYUV420(uint8_t * input, uint8_t * output, int width, int height, cudaStream_t stream=NULL, int matrix = 0);
cudaError_t cudaRGBToYUV420(uint8_t * input, uint8_t * output, int width, int height, cudaStream_t stream=NULL, int matrix = 0);
// } // namespace jetson_encoder_compressed_image_transport
