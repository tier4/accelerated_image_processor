...
: Node("gpu_imgproc", options), rectifier_active_(false)
{
  ...
  // std::string image_raw_topic = this->declare_parameter<std::string>("image_raw_topic",
  // "/camera/image_raw"); std::string camera_info_topic =
  // this->declare_parameter<std::string>("camera_info_topic", "/camera/camera_info"); std::string
  // image_rect_topic = this->declare_parameter<std::string>("image_rect_topic",
  // "/camera/image_rect");
  ...
  // NOTE: too short `max_task_queue_length_` may cause topic drop, while too large one may cause
  // 100% system memory usage under many cameras/high framerate conditions
  max_task_queue_length_ =
    static_cast<size_t>(this->declare_parameter<int64_t>("max_task_queue_length", 5));
  ...
  if (available_impls == "") {
    RCLCPP_ERROR(
      this->get_logger(),
      "No rectification implementations available. Please make sure that at least one of the "
      "following libraries is installed:\n"
      "- OpenCV\n"
      "- OpenCV CUDA\n"
  ...
  if (0) {
    ...
    RCLCPP_ERROR(
      this->get_logger(), "Invalid implementation: %s. Available options: %s", rect_impl.c_str(),
      available_impls.c_str());
    return;
  }
  ...
  qos_request_timer_ = rclcpp::create_timer(
    ...
    [this]() { this->determineQosCallback(do_rectify_); });
}
...
void GpuImgProc::determineQosCallback(bool do_rectify)
{
  ...
  std::string img_sub_topic_name =
    this->get_node_topics_interface()->resolve_topic_name("image_raw", false);
  std::string info_sub_topic_name =
    this->get_node_topics_interface()->resolve_topic_name("camera_info", false);
  ...
  compressed_pub_ =
    this->create_publisher<sensor_msgs::msg::CompressedImage>("image_raw/compressed", img_qos);
  ...
  img_sub_ = std::make_shared<CudaBlackboardSubscriber>(
    *this, "image_raw", false, std::bind(&GpuImgProc::imageCallback, this, std::placeholders::_1));

  if (do_rectify) {
    // rectified_pub_ = this->create_publisher<sensor_msgs::msg::Image>("image_rect", img_qos);
    rectified_pub_ = std::make_shared<CudaBlackboardPublisher>(*this, "image_rect"); 
    rect_compressed_pub_ =
      this->create_publisher<sensor_msgs::msg::CompressedImage>("image_rect/compressed", img_qos);
    ...
    info_sub_ = this->create_subscription<sensor_msgs::msg::CameraInfo>(
      info_sub_topic_name, info_qos,
      std::bind(&GpuImgProc::cameraInfoCallback, this, std::placeholders::_1));
  }
...
void GpuImgProc::imageCallback(std::shared_ptr<const CudaImage> msg)
{
  ...
  if (msg->encoding == "rgb8") {
    ...
    RCLCPP_ERROR_STREAM(
      this->get_logger(), "Image encoding (" << msg->encoding << ") is not supported.");
  }
...
void GpuImgProc::cameraInfoCallback(const sensor_msgs::msg::CameraInfo::SharedPtr msg)
{
  ...
  switch (rectifier_impl_) {
    ...
    case Rectifier::Implementation::OpenCV_CPU:
      ...
      cv_cpu_rectifier_ =
        std::make_shared<Rectifier::OpenCVRectifierCPU>(*msg, mapping_impl_, alpha_);
    ...
    case Rectifier::Implementation::OpenCV_GPU:
      ...
      cv_gpu_rectifier_ =
        std::make_shared<Rectifier::OpenCVRectifierGPU>(*msg, mapping_impl_, alpha_);
