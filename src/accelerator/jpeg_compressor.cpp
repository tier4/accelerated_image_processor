...
CompressedImage::UniquePtr JetsonCompressor::compress(const ImageContainer &msg, int quality, int format, int sampling)
{
    stream_ = msg.cuda_stream()->stream();
    int width = msg.width();
    int height = msg.height();
    // const auto &img = msg.data;
    size_t out_buf_size = width * height * 3 / 2;
    ...
    NppiSize roi = {static_cast<int>(msg.width), static_cast<int>(msg.height)};
    if (format == ImageFormat::RGB) {
        TEST_ERROR(nppiRGBToYUV420_8u_C3P3R_Ctx(msg.cuda_mem(), dev_image_step_bytes_,
                                                dev_yuv_.data(), dev_yuv_step_bytes_.data(), roi,
                                                npp_stream_context_) != NPP_SUCCESS,
                   "failed to convert rgb8 to yuv420");
    } else {
